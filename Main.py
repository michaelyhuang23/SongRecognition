...
class Predictor:
    ...
    def get_tally_winner(self):
        return self.pollster.most_common()[0]
        
    def add_song(self, file_path : str, songname : str, artist : str, duration : int):
        audio = read_audio()
        # these should read in discrete digital data
        spectro = spectrogram(audio)
        # returns (Frequency, Time) data
        peaks = get_peaks(spectro)
        self.songs.save_song(peaks, songname, artist, duration)
        for peak in peaks:
    ...
    def load_song(self, file_path: str):
        samples, sampling_rate = librosa.load(file_path, sr=44100, mono=True, duration=length)
